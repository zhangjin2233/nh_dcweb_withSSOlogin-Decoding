<template>
    ...
    <el-tabs type="card">
        ...
        <dc-table v-if="demo.tabs[i].type == 'Table'" :object="table"></dc-table>
	  		<!-- 文本类型 -->
	  		<el-input type="textarea" :rows="9" v-else-if="demo.tabs[i].type == 'String'" v-model="tab.value" :readonly="demo.tabs[i].readOnly"></el-input>
			</el-tab-pane>
		</el-tabs>
...
import saveNode from '@/assets/js/flow/saveNode.js'
...
export default {
  ...
  data () {
    return {
      ...
      table: new dc.Table({
        ...
        hiddenCols: ['name','size','canBeEmpty'],
        hasPage: false,
        editable: true,
      }),
      loading: false,
      ...
      btns: [{
        ...
        hide: true,
        click: ()=>{
          JSON.stringify(this.fromQuery) != '{}' ? this.$router.push({path: '/sdf', query: this.fromQuery}) : this.$router.go(-1);
        }
      }]
    }
  },
  ...
  watch:{
    ...
    newNode(val) {
      if(this.$route.path != '/sdc') return;
      console.log("run");
      let node = Tree.getNode(this.$route.query.link);
      let nodeData = node.data.children.find(item=>item.label == this.attrData.head.label || this.attrData.head.name);
      // let nodeData = {};
      ...
      // if(JSON.stringify(nodeData) != '{}'){
        this.$router.replace({ query:merge({},objClone(nodeData,{},['id','pId','link','label','type','storyId'],true)) })
      // }
    },
  },
  computed: {
    ...
    refresh(Vue,data){
      ...
      if(Vue.formId && Vue.attrData.head.label != Vue.$route.query.label){
        ...
        Tree.refreshTree(this.$route.query.link).then(res=>{
          let nodeData = res.find(item=>(item.label == (Vue.attrData.head.label || Vue.attrData.head.name)) && (item.type=='SDC'))
          if(nodeData){
